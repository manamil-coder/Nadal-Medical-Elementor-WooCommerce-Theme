...
if ( ! class_exists( 'Medizin_WP_Widget_Product_Layered_Nav' ) ) {
	class Medizin_WP_Widget_Product_Layered_Nav extends Medizin_Widget {
		...
		public function __construct() {
			...
			$this->widget_id          = 'medizin-wp-widget-product-layered-nav';
			$this->widget_cssclass    = 'medizin-wp-widget-product-layered-nav';
			$this->widget_name        = esc_html__( '[Medizin] Product Attribute Layered Nav', 'medizin' );
			$this->widget_description = esc_html__( 'Display a list of attributes to filter products in your store.', 'medizin' );
			$this->settings           = array(
				'title'        => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Filter by', 'medizin' ),
					'label' => esc_html__( 'Title', 'medizin' ),
				),
				'attribute'    => array(
					'type'    => 'select',
					'std'     => '',
					'label'   => esc_html__( 'Attribute', 'medizin' ),
					'options' => $attribute_array,
				),
				'query_type'   => array(
					'type'    => 'select',
					'std'     => 'and',
					'label'   => esc_html__( 'Query type', 'medizin' ),
					'options' => array(
						'and' => esc_html__( 'AND', 'medizin' ),
						'or'  => esc_html__( 'OR', 'medizin' ),
					),
				),
				'display_type' => array(
					'type'    => 'select',
				...
				'labels'       => array(
					'type'    => 'select',
					'std'     => 'on',
					'label'   => esc_html__( 'Show labels', 'medizin' ),
					'options' => array(
						'on'  => esc_html__( 'ON', 'medizin' ),
						'off' => esc_html__( 'OFF', 'medizin' ),
					),
				),
				'items_count'  => array(
					'type'    => 'select',
					'std'     => 'on',
					'label'   => esc_html__( 'Show items count', 'medizin' ),
					'options' => array(
						'on'  => esc_html__( 'ON', 'medizin' ),
						'off' => esc_html__( 'OFF', 'medizin' ),
					),
		...
		protected function layered_nav_dropdown( $terms, $taxonomy, $query_type ) {
			...
			if ( $taxonomy !== $this->get_current_taxonomy() ) {
				...
				$any_label            = apply_filters( 'woocommerce_layered_nav_any_label',
					sprintf( esc_html__( 'Any %s', 'medizin' ), $taxonomy_label ),
					$taxonomy_label,
					$taxonomy );
				...
				echo '<select class="dropdown_layered_nav_' . $taxonomy_filter_name . '" data-filter-url="' . preg_replace( '%\/page\/[0-9]+%',
						...
						str_replace( array(
							...
							esc_js( add_query_arg( 'filtering',
								...
								) ) ) ) ) ) . "&filter_" . esc_js( $taxonomy_filter_name ) . "=medizin_FILTER_VALUE" . '">';
		...
		protected function layered_nav_list( $terms, $taxonomy, $query_type, $instance ) {
			...
			foreach ( $terms as $term ) {
				 ...
				 */
				$term_html = apply_filters( 'medizin_woocommerce_layered_nav_term_html', $term_html, $term, $link, $count );
