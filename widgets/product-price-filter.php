...
if ( ! class_exists( 'Medizin_WP_Widget_Product_Price_Filter' ) ) {
	class Medizin_WP_Widget_Product_Price_Filter extends Medizin_Widget {

		private $_check_ranges = false;

		public function __construct() {
			$this->widget_id          = 'medizin-wp-widget-product-price-filter';
			$this->widget_cssclass    = 'medizin-wp-widget-product-price-filter';
			$this->widget_name        = esc_html__( '[Medizin] Product Price Filter', 'medizin' );
			$this->widget_description = esc_html__( 'Display a price range list to filter products in your store.', 'medizin' );
			$this->settings           = array(
				'title' => array(
					'type'  => 'text',
					'std'   => esc_html__( 'Price Filter', 'medizin' ),
		...
		public function widget( $args, $instance ) {
			...
			if ( ! empty( $links ) ) {
				?>
				<div class="medizin-product-price-filter">
					<ul>
		...
		private function generate_price_links( $min, $max, $min_price, $max_price ) {
			...
			$steps = apply_filters( 'medizin_price_filter_steps', 5 );
