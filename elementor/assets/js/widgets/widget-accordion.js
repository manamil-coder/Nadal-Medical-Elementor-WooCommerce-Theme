(
	function( $ ) {
		...
		var MedizinAccordionHandler = function( $scope, $ ) {
			var $element = $scope.find( '.medizin-accordion' );
			...
			$element.on( 'click', '.accordion-header', function( e ) {
				...
				if ( section.hasClass( 'active' ) ) {
					...
					var parent = $( this ).parents( '.medizin-accordion' ).first();
		...
		$( window ).on( 'elementor/frontend/init', function() {
			elementorFrontend.hooks.addAction( 'frontend/element_ready/tm-accordion.default', MedizinAccordionHandler );
		} );
	}
)( jQuery );
