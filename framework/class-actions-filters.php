 ...
 */
if ( ! class_exists( 'Medizin_Actions_Filters' ) ) {
	class Medizin_Actions_Filters {
		...
		public function initialize() {
			...
			add_filter( 'medizin_body_attributes', array( $this, 'add_attributes_to_body' ) );
		...
		function change_breadcrumb_text( $args ) {
			$args['home_label']   = esc_html__( 'Home', 'medizin' );
			$args['search_label'] = esc_html__( 'Search Result of &quot;%s&quot;', 'medizin' );
			$args['404_label']    = esc_html__( '404 Not Found', 'medizin' );
		...
		public function add_extra_user_info( $fields ) {
			$new_fields = array(
				array(
					'name'  => 'phone_number',
					'label' => esc_html__( 'Phone Number', 'medizin' ),
				),
				array(
					'name'  => 'career',
					'label' => esc_html__( 'Career', 'medizin' ),
				),
				array(
					'name'  => 'email_address',
					'label' => esc_html__( 'Email Address', 'medizin' ),
				),
				array(
					'name'  => 'facebook',
					'label' => esc_html__( 'Facebook', 'medizin' ),
				),
				array(
					'name'  => 'twitter',
					'label' => esc_html__( 'Twitter', 'medizin' ),
				),
				array(
					'name'  => 'instagram',
					'label' => esc_html__( 'Instagram', 'medizin' ),
				),
				array(
					'name'  => 'linkedin',
					'label' => esc_html__( 'Linkedin', 'medizin' ),
				),
				array(
					'name'  => 'pinterest',
					'label' => esc_html__( 'Pinterest', 'medizin' ),
				),
				array(
					'name'  => 'youtube',
					'label' => esc_html__( 'Youtube', 'medizin' ),
				),
			);
		...
		public function alter_search_loop( $query ) {
			if ( $query->is_main_query() && $query->is_search() ) {
				$number_results = Medizin::setting( 'search_page_number_results' );
		...
		public function search_filter( $query ) {
			if ( $query->is_main_query() && $query->is_search ) {
				$post_type = Medizin::setting( 'search_page_filter' );
				if ( $post_type !== 'all' ) {
		...
		public function add_wrapper_for_video( $html, $url ) {
			...
			if ( Medizin_Helper::strposa( $url, $array ) ) {
		...
		public function add_attributes_to_body( $attrs ) {
			...
			$font = Medizin_Helper::get_body_font();
		...
		public function body_classes( $classes ) {
			...
			$mobile_menu_effect = Medizin::setting( 'mobile_menu_effect' );
			$classes[]          = "mobile-menu-{$mobile_menu_effect}";

			if ( '1' === Medizin::setting( 'top_bar_hide_on_mobile' ) ) {
			...
			if ( Medizin_Woo::instance()->is_activated() ) {
				...
				if ( '1' === Medizin::setting( 'shop_archive_thumbnail_background' ) ) {
				...
				if ( Medizin_Woo::instance()->is_product_archive() ) {
					...
					$archive_shop_layout = Medizin::setting( 'shop_archive_layout' );
					$classes[]           = "archive-shop-{$archive_shop_layout}";
				}

				if ( is_singular( 'product' ) ) {
					$product_feature_style = Medizin_Woo::instance()->get_single_product_style();
					$classes[]             = "single-product-{$product_feature_style}";
				}

				if ( '1' === Medizin::setting( 'shop_archive_hide_buttons_on_mobile' ) ) {
					$classes[] = 'woocommerce-loop-buttons-hide-on-mobile';
				}
			}

			if ( Medizin_Post::instance()->is_archive() ) {
				$blog_archive_style = Medizin::setting( 'blog_archive_style' );
				$classes[]          = "blog-archive-style-{$blog_archive_style}";
			}

			$one_page_enable = Medizin_Helper::get_post_meta( 'menu_one_page', '' );
			if ( $one_page_enable === '1' ) {
				$classes[] = 'one-page';
			}

			if ( is_singular( 'portfolio' ) ) {
				$skin = Medizin_Helper::get_post_meta( 'portfolio_site_skin', '' );
				if ( $skin === '' ) {
					$skin = Medizin::setting( 'single_portfolio_site_skin' );
				}
			...
			$header_sticky_behaviour = Medizin::setting( 'header_sticky_behaviour' );
			$classes[]               = "header-sticky-$header_sticky_behaviour";

			$site_layout = Medizin_Helper::get_post_meta( 'site_layout', '' );
			if ( $site_layout === '' ) {
				$site_layout = Medizin::setting( 'site_layout' );
			}
			$classes[] = $site_layout;

			$site_class = Medizin_Helper::get_post_meta( 'site_class', '' );
			if ( $site_class !== '' ) {
				$classes[] = $site_class;
			}
	...
	Medizin_Actions_Filters::instance()->initialize();
}
