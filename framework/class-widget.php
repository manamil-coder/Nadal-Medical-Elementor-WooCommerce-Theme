 ...
 * @extends  WP_Widget
 */
if ( ! class_exists( 'Medizin_Widget' ) ) {
	abstract class Medizin_Widget extends WP_Widget {
		...
		public function form( $instance ) {
			...
			foreach ( $this->settings as $key => $setting ) {
				...
				switch ( $setting['type'] ) {
					...
					case 'image':
						...
						<div class="kungfu-attach-wrap">
							...
							<a class="kungfu-media-open kungfu-button success">
								<i class="fa fa-upload"></i><?php esc_html_e( 'Upload', 'medizin' ); ?>
							</a>
							<a class="kungfu-media-remove kungfu-button danger"
								<?php if ( empty( $value ) ) : ?>
									style="display:none"
								<?php endif; ?>
							><i class="fa fa-trash-o"></i><?php esc_html_e( 'Remove', 'medizin' ); ?></a>
						</div>
						<?php
						break;
					...
					default :
						do_action( 'medizin_widget_field_' . $setting['type'], $key, $value, $setting, $instance );
						break;
				}
			}
		}
